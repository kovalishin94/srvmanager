...
class CoreTestCase(BaseTestCase):
    ...
    def test_ssh_credential_delete(self):
        ssh_credential_id = self.create_ssh_credential().data.get('id')
        response = self.client.delete(
            reverse('ssh-credential-detail', args=[ssh_credential_id]))
    ...
    def test_ssh_credential_update(self):
        ...
        response = self.client.put(
            reverse('ssh-credential-detail', args=[ssh_credential_id]), data)
    ...
    def test_winrm_credential_delete(self):
        winrm_credential_id = self.create_winrm_credential().data.get('id')
        response = self.client.delete(
            reverse('winrm-credential-detail', args=[winrm_credential_id]))
        self.assertEqual(response.status_code, 204)
    ...
    def test_winrm_credential_update(self):
        ...
        response = self.client.put(
            reverse('winrm-credential-detail', args=[winrm_credential_id]), data)
        winrm_credential = WinRMCredential.objects.get(id=winrm_credential_id)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data.get('username'), 'user_edited')
    ...
    def test_host_delete(self):
        host_id = self.create_host().data.get('id')
        response = self.client.delete(reverse('host-detail', args=[host_id]))
        self.assertEqual(response.status_code, 204)

    def test_host_update(self):
        host_id = self.create_host().data.get('id')
        data = {'name': 'edited_host', 'ip': '192.168.99.99', 'os': 'windows'}
        response = self.client.put(reverse('host-detail', args=[host_id]), data)
        self.assertEqual(response.status_code, 200)
